...
<div id='contenu' >
  ...
  ?>
    ...
    if ($etape == 'affichageFiche'){
      ...
      <a href="?etape=modificationFiche&amp;id=<?php echo $idVisiteur; ?>"
         onclick="return confirm('Voulez-vous vraiment modifier le contenu de cette fiche ?');"
         title="Modifier la fiche de frais">modifier</a>
      <?php
    }
    ?>
   ...
   <h3> Frais Hors Forfait </h3>
    ...
    if( $etape == 'affichageFiche'){
      if($etat == 'CL'){
        ?>
        <h2><a href=?id=<?php echo $idVisiteur ?>&etape=ficheValide> VALIDER </a> <a href=cValidationFicheFrais.php? > RETOUR </a></h2>
        <?php
      }
